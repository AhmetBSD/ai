...
class AuthExpiredError(PanosError):
    """PA rejected the credentials — usually because the API key has expired
    (api-key-lifetime exceeded) or was generated by a now-deleted user.
    The customer-facing remediation is to re-run auth.py.
    """


def _is_auth_failure(exc: Exception) -> bool:
    """Heuristic: PAN-OS surfaces auth/key problems through a handful of
    error strings depending on SDK / API path. Catch the common ones."""
    s = str(exc).lower()
        ...
        try:
            ...
            if _is_auth_failure(e):
                raise AuthExpiredError(
                    "PAN-OS kimlik bilgisi reddedildi (API key süresi dolmuş veya "
                    "kullanıcı kaldırılmış olabilir). Çözüm: `auth.py` çalıştır, "
                    "host/kullanıcı/şifre gir; skill yeni API key alacak."
                ) from e
            # Any other pan-os-python / network exception (PanConnectionTimeout,
            # PanDeviceError, URLError, ...) is wrapped so entry-points see a
            # single uniform PanosError instead of a Python traceback.
            raise PanosError(f"PAN-OS API çağrısı başarısız: {e}") from e
